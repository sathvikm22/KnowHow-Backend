...
const FIRST_PARTY_HOST_SUFFIX = 'knowhowindia.in';
const ACCESS_COOKIE_MAX_AGE_MS = 60 * 60 * 1000; // 1 hour (matches JWT)

/** First-party when frontend proxies /api via Vercel (X-Forwarded-Host) or local Vite proxy */
const isFirstPartyRequest = (req) => {
  if (!req) return false;
  const host = (req.get('x-forwarded-host') || req.get('host') || '').toLowerCase();
  return host.includes(FIRST_PARTY_HOST_SUFFIX) || host.startsWith('localhost') || host.startsWith('127.0.0.1');
};

const getAuthCookieOptions = (req, maxAge) => {
  const firstParty = isFirstPartyRequest(req);
  const isHttps =
    req &&
    (req.secure ||
      req.protocol === 'https' ||
  ...
  return {
    httpOnly: true,
    secure: firstParty ? Boolean(isHttps) : true,
    sameSite: firstParty ? 'lax' : 'none',
    maxAge,
    path: '/',
  };
};
...
const setAuthCookies = (res, userId, email, req = null) => {
  ...
  const accessCookieOptions = getAuthCookieOptions(req, ACCESS_COOKIE_MAX_AGE_MS);
  const refreshCookieOptions = getAuthCookieOptions(req, 7 * 24 * 60 * 60 * 1000);
  ...
  console.log('🍪 Setting cookies with options:', {
    ...
    firstParty: isFirstPartyRequest(req),
...
// Helper function to clear auth cookies (both first-party and legacy cross-site variants)
const clearAuthCookies = (res, req = null) => {
  const primary = getAuthCookieOptions(req, 0);
  const { maxAge: _maxAge, ...clearPrimary } = primary;
  res.clearCookie('accessToken', clearPrimary);
  res.clearCookie('refreshToken', clearPrimary);

  if (primary.sameSite !== 'none') {
    const crossSite = { httpOnly: true, secure: true, sameSite: 'none', path: '/' };
    res.clearCookie('accessToken', crossSite);
    res.clearCookie('refreshToken', crossSite);
  }
};
...
router.get('/google/callback', async (req, res) => {
  try {
    ...
    if (!user || !user.id || !user.email) {
      console.error('CRITICAL: User object is invalid after creation/lookup:', user);
      return res.redirect(`${frontendUrl}/login?error=user_creation_failed`);
    }
